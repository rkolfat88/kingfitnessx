...
export default function MindGym({ mindProfile, onRefresh, onNavigateToCheckin }: MindGymProps) {
  ...
  const getScoreLabel = (score: number) => {
    ...
    if (score >= 60) return { label: 'Building', color: '#CAFF40' };
  ...
  const getMotivationMessage = (level: number) => {
    ...
    if (level <= 6) return { msg: "Steady state. Consistency over intensity today.", color: '#CAFF40', mode: 'steady' };
  ...
  return (
    <div className="min-h-screen bg-[#000000] pb-24">
      <div className="px-5 pt-12 pb-4">
        <p className="text-xs font-semibold uppercase tracking-widest text-[#A0A0A0] mb-1">Mind Gym</p>
        <h1 className="text-3xl font-black text-[#FFFFFF]">Mental Performance</h1>
      </div>
      ...
      <div className="px-5 space-y-4">
        ...
        <div className="bg-[#0D0D0D] border border-[#CAFF40]/20 rounded-2xl p-5">
          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-[#A0A0A0]">Mind Score</p>
            <button className="text-xs text-[#CAFF40]">Details →</button>
          </div>
          <div className="flex items-center gap-4">
            <div className="relative w-16 h-16">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <span className="text-sm font-black text-[#FFFFFF]">{getMindScore()}</span>
              </div>
            </div>
            <div>
              <p className="text-xl font-black" style={{ color: scoreInfo.color }}>{scoreInfo.label}</p>
              <p className="text-xs text-[#A0A0A0] mt-1">"{mindProfile?.identity_statement || 'Set your identity in Phase Zero'}"</p>
            </div>
          </div>
        </div>
        ...
        {!submitted ? (
          <div className="bg-[#0D0D0D] border border-[#CAFF40]/20 rounded-2xl p-5">
            <p className="text-sm font-semibold text-[#FFFFFF] mb-1">How's your mind today?</p>
            <p className="text-xs text-[#A0A0A0] mb-4">Be honest. This changes your entire program.</p>

            <div className="flex items-center justify-between mb-2">
              <span className="text-xs text-[#5C5C5C]">Barely here</span>
              <span className="text-lg font-black" style={{ color: motivationInfo.color }}>{motivationLevel}/10</span>
              <span className="text-xs text-[#5C5C5C]">Locked in</span>
            </div>
            ...
            <input
              ...
              style={{ accentColor: '#CAFF40', background: '#141414' }}
            />

            <div className="bg-[#000000] rounded-xl px-3 py-2 mb-4">
            ...
            <button
              ...
              disabled={loading}
              className="w-full bg-[#CAFF40] hover:bg-[#A8D930] disabled:opacity-40 text-black font-bold py-3 rounded-xl text-sm transition-all"
            >
        ...
        {/* AI Primer */}
        {aiPrimer && (
          <div className="bg-[#0D0D0D] border border-[#CAFF40]/30 rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-3">
              <div className="w-7 h-7 rounded-full bg-[#CAFF40]/20 border border-[#CAFF40]/30 flex items-center justify-center">
                <Brain className="w-3.5 h-3.5 text-[#CAFF40]" />
              </div>
              <span className="text-xs font-semibold text-[#CAFF40]">Today's Mental Primer</span>
            </div>
            <p className="text-sm text-[#FFFFFF] leading-relaxed">{aiPrimer}</p>
          </div>
        )}
        ...
        {aiPrimer && (
          <button
            ...
            className="w-full bg-[#0D0D0D] border border-[#CAFF40]/20 rounded-2xl p-4 flex items-center justify-between"
          >
            <div className="text-left">
              <p className="text-sm font-bold text-[#FFFFFF]">Log today's readiness</p>
              <p className="text-xs text-[#A0A0A0]">Adapts today's training session</p>
            </div>
            <ChevronRight className="w-4 h-4 text-[#CAFF40]" />
          </button>
        )}
        ...
        {mindProfile?.identity_statement && (
          <div className="bg-[#CAFF40]/5 border border-[#CAFF40]/20 rounded-2xl p-4">
            <p className="text-xs font-semibold uppercase tracking-widest text-[#CAFF40] mb-2">Your Identity</p>
            <p className="text-base font-semibold text-[#FFFFFF] italic">"{mindProfile.identity_statement}"</p>
          </div>
        )}

        {/* Mind Tools */}
        <div>
          <p className="text-xs font-semibold uppercase tracking-widest text-[#A0A0A0] mb-3">Mind Tools</p>
          <div className="space-y-2">
            {[
              ...
              { icon: Heart, label: "Self-Compassion Reset", desc: "Stop punishing, start building", color: "#CAFF40", tool: 'compassion' as ActiveTool },
            ].map(item => (
              <button
                key={item.label}
                onClick={() => setActiveTool(item.tool)}
                className="w-full bg-[#0D0D0D] border border-[#262626] hover:border-[#CAFF40]/30 rounded-xl p-4 flex items-center gap-3 transition-all text-left"
              >
