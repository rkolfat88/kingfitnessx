  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#000000] flex items-center justify-center">
        <Apple className="w-8 h-8 text-[#CAFF40] animate-pulse" />
      </div>
    );
  }
  ...
  if (!macros) {
    return (
      <div className="min-h-screen bg-[#000000] flex flex-col items-center justify-center px-5 text-center">
        <Apple className="w-10 h-10 text-[#5C5C5C] mb-4" />
        <h2 className="text-xl font-black text-[#FFFFFF] mb-2">No Plan Yet</h2>
        <p className="text-sm text-[#A0A0A0]">Complete onboarding to generate your nutrition plan.</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#000000] pb-24">
      <div className="px-5 pt-12 pb-4">
        ...
        <p className="text-sm text-[#CAFF40] font-semibold">
      ...
      <div className="px-5 space-y-4">
        ...
        <div className="bg-[#0D0D0D] border border-[#CAFF40]/20 rounded-2xl p-5">
          ...
          <div className="text-center mb-6">
            <p className="text-5xl font-black text-[#FFFFFF]">{macros.calories}</p>
            <p className="text-sm text-[#A0A0A0] mt-1">calories / day</p>
          </div>

          <div className="space-y-4">
            {[
              { label: 'Protein', grams: macros.protein_g, cals: proteinCals, color: '#CAFF40', note: 'LOCKED — muscle preservation' },
              ...
              <div key={macro.label}>
                <div className="flex items-center justify-between mb-1.5">
                  <div>
                    <span className="text-sm font-semibold text-[#FFFFFF]">{macro.label}</span>
                    {macro.note && (
                      <span className="text-xs text-[#5C5C5C] ml-2">{macro.note}</span>
                    )}
                  </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-[#FFFFFF]">How were these calculated?</span>
          </div>
          ...
          <div className="bg-[#0D0D0D] border border-[#CAFF40]/20 rounded-2xl p-4 space-y-2">
            {reasoning.map((r, i) => (
              <div key={i} className="flex gap-2">
                ...
                <p className="text-xs text-[#A0A0A0] leading-relaxed font-mono">{r}</p>
              </div>
            ))}
          </div>
        )}
          ...
          <div className="space-y-2">
            {[
              ...
              <div key={item.time} className="flex gap-3">
                <span className="text-xs font-bold text-[#CAFF40] w-24 flex-shrink-0 mt-0.5">{item.time}</span>
                <p className="text-xs text-[#A0A0A0] leading-relaxed">{item.note}</p>
              </div>
            ))}
          </div>
        </div>
